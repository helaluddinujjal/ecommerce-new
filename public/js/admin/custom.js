 ...
 */
    ...
    */
    ...
    //state check
    $(document).on('click', '#stateCheck', function () {
    ...
    //is acceptable pincode or not
    $(document).on('click', '#is_accept', function () {
        if (this.checked) {
            $("label[for='acceptable_pincode']").html("Acceptable Pincode (seperate by coma(,). ex: 4334,5554)")
        } else {
            $("label[for='acceptable_pincode']").html("Unacceptable Pincode (seperate by coma(,). ex: 4334,5554)")
        }
    })
})
/*
...
function getState(country_id) {
    ...
    $.ajax({
        ...
        success: function (res) {
            $("#stateSection").find("#state:visible").find('option').remove().end()
            $("#stateSection").find("#state:visible").append('<option value="">Select State</option>');
            $.each(res.states, function (key, value) {
                $("#stateSection").find('#state:visible').append('<option value="' + value['id'] + '">' + value['name'] + '</option>');
            })
        },
        error: function () {
            Toast.fire({
...
function getCity(state_id) {
    ...
    $.ajax({
        ...
        success: function (res) {
            $("#citySection").find("#city:visible").find('option').remove().end()
            $("#citySection").find("#city:visible").append('<option value="">Select City</option>');
            $.each(res.cities, function (key, value) {
                $("#citySection").find('#city:visible').append('<option value="' + value['id'] + '">' + value['name'] + '</option>');
            })
