        ...
        if ($request->isMethod('post')) {
            ...
            if (Auth::attempt(['email' => $data['login_email'], 'password' => $data['login_password']])) {
                ...
                if ($statusCheck->status == 0) {
                    Auth::logout();
                    toast('Your account is not activated yet!! Please confirm your email to activate!', 'error');
                ...
                if ($statusCheck->status == 2) {
                    Auth::logout();
                    toast('Your account is disabled by admin! Please contact to admin!', 'error');
                    // Session::flash('error_msg', "Your account is deactivate by admin. Please contact with admin!");
                    return redirect()->back();
                }
