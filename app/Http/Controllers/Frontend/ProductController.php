    ...
    public function addToCart(Request $request){
        if ($request->isMethod('post')) {
            ...
            if ($productAttr->stock<$data['quantity']) {
                ...
                if (Auth::check()) {
                    $countCart=Cart::where('product_id',$data['product_id'])->where(function($query) use($session_id){
                        $query->where('user_id',Auth::user()->id);
                        $query->orWhere('session_id',$session_id);
                    })->count();
                }else{
                    $countCart=Cart::where(['product_id'=>$data['product_id'],'session_id'=>$session_id])->count();
                }
                if ($countCart>0) {
    ...
    public function checkout(Request $request){
        ...
        if ($request->isMethod('post')) {
            $rules=[
                ...
                "payment_gatway"=>"required|in:Paypal,COD",
            ];
